...
*******************************************************************************/
/******************************************************************************/
/** \file system_mb9a310k.h
 **
 ** Headerfile for FM3 system parameters
 **
 ** Version:
 ** Ver 2.0 MISH Auto created by make header script
 ** Ver 2.1 NOSU Updated Disclaimer
 ** Ver 2.2 NOSU Added code to select PLL input clock by PINC bit of PSW_TMR
 **              Check range of PLL oscillation frequency when PLL is enabled.
 ** Timestamp:
 ** 20.5.2016 11:51:56 2.2 MISH Auto created by make header script
 **
 **
 ...
 ** - 1: CLOCK_SETTING_CMSIS - Clock setting done in system_mb9a310k.h like in
 **                            FM3 template projects
 ******************************************************************************/ 
 ...
 ** - CLOCK_SETTING_CMSIS - Clock setting done in system_mb9a310k.h like in
 **                         FM3 template projects
 ******************************************************************************/ 
 ...
 ******************************************************************************/    
#define PLL_CTL1_Val           ( 0x00000004ul)   // <<< Define PLL_CTL1 here
 ...
 **        <i>(DEVICE DEPENDENT SETTING)</i>
 ******************************************************************************/ 
#define __PLLCLKOUTMAX  ( 300000000ul)
...
/**
 ******************************************************************************
 ** \brief Define PLL Input Clock from settings
 ******************************************************************************/
#if   (((PSW_TMR_Val >> 4ul) & 0x01UL) == 0ul)
  #define __PINCLK     (__CLKMO)
#elif (((PSW_TMR_Val >> 4ul) & 0x01UL) == 1ul)
  #define __PINCLK     (__CLKHC)
#else
  #define __PINCLK     (0UL)
#endif
 ...
 ******************************************************************************/
#define __PLLCLK       ((__PINCLK  * __PLLN) / __PLLK)
...
#if (CHECK_RANGE((__PINCLK / __PLLK), __PLLCLKINMIN, __PLLCLKINMAX))
#if ((SCM_CTL_Val & 0x10ul) == 0x10ul))
  #error "PLL input frequency out of range."
#else
  #warning "PLL input frequency out of range.  Review PLL factors if PLL is used."
#endif  
#endif  

#if (CHECK_RANGE(((__PINCLK * __PLLN * __PLLM) / __PLLK), __PLLCLKOUTMIN, __PLLCLKOUTMAX))
#if ((SCM_CTL_Val & 0x10ul) == 0x10ul))
  #error "PLL oscillation frequency out of range."
#else
  #warning "PLL oscillation frequency out of range.  Review PLL factors if PLL is used."
#endif  
#endif  
