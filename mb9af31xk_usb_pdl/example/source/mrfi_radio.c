 ...
 */
void __inline MRFI_RSSI_VALID_WAIT(void){                                     
...
#define SMARTRF_SETTING_IOCFG0     0x0C
...
#define SMARTRF_SETTING_MDMCFG4    0xA5
...
#define PA_TABLE_OOK {0xC0, 0xC0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}
static uint8_t mrfiRFPowerTable[] = PA_TABLE_OOK;

/* ------------------------------------------------------------------------------------------------
 *                                       Local Prototypes
 * ------------------------------------------------------------------------------------------------
 */
BSP_ISR_FUNCTION( MRFI_GpioIsr, 0 );			  
//void MRFI_GpioIsr(void); 	

void Mrfi_RxModeOn(void);	/*  this called from mrfi_f1f2.c */	
void Mrfi_RxModeOff(void);	/* this called from mrfi_f1f2.c */

static void __inline Mrfi_SyncPinRxIsr(void);
 ...
 */
//static 
volatile uint8_t mrfiRadioState  = MRFI_RADIO_STATE_UNKNOWN;
volatile mrfiPacket_t mrfiIncomingPacket;
...
volatile bool 	rx_isr_context = false;
...
void MRFI_Init(void)
{
  ...
  MRFI_POWER_RX_MODE();		//Just to enable RSSI reception
...
#define   PHY_PREAMBLE_SYNC_BYTES    8
   ...
   */
  MRFI_CONFIG_SYNC_PIN_AS_SYNC_SIGNAL();
...
//    MRFI_CompleteTxPrep( pPacket );
    ...
    MRFI_CompleteTxPrep( pPacket );
  	...
  	MRFI_POWER_TX_MODE();	//Setup power module on TX Mode
  ...
  else
  {
     ...
     */
    MRFI_CONFIG_SYNC_PIN_AS_PAPD_SIGNAL();
...
bool Mrfi_DelayUsecLong(uint32_t ms, uint16_t us, TimeoutTerminator_t term)
{
  ...
  if( ms != 0 )
  {
    ...
    do // wait for milliseconds to timeout
    {
      ...
      if( BSP_INTERRUPTS_ARE_DISABLED() && BSP_TIMER_CHECK_OVERFLOW_FLAG( ) )
      {
