...
{
    enSelectedDriver = UsbHostNdisTypeUknown;
    memset(au8ManagementElementNotifications,0,8);
    ControlEndpointIN = NULL;
    DataEndpointIN = NULL;
    DataEndpointOUT = NULL;
    ...
    u32MaxRxSize = USBHOSTNDIS_MAXIMUM_RECEIVE_BUFFER;
    USB_ZERO_STRUCT(stcUsbHostNdisMacAddress);
}
...
void UsbHostNdis_RequestMacAddressPolled(uint8_t u8IndexStringDescriptor)
{
    ...
    while((u32Timeout > 0) && (bMacAddressReturned == FALSE)) 
    {
        Usb_WaitHook();
        u32Timeout--;
    }
...
boolean_t UsbHostNdis_InitHandler(stc_usbn_t* pstcUsb, uint8_t* pu8Configuration, uint32_t u32Length)
{
    ...
    {
        UsbHostNdis_DeinitHandler(pstcUsb);
        return FALSE;
    }
}
