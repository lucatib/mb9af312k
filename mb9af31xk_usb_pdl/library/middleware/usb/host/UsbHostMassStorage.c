...
static const stc_usbhostclasssupervisor_usbclassdriver_t stcDriverSetup = {
    UsbHostMassStorage_InitHandler,   //pfnInitClassDriver           deinitialization callback of class driver
    UsbHostMassStorage_DeinitHandler, //pfnDeinitClassDriver         initialization callback of class driver
    ...
    8,                               //u8MatchInterfaceClass        match interface class
    6,                                //u8MatchInterfaceSubClass     match interface subclass
    80,                               //u8MatchInterfaceProtocol     match interface protocoll
...
boolean_t UsbHostMassStorage_InitHandler(stc_usbn_t* pstcUsb, uint8_t* pu8Configuration, uint32_t u32Length)
{
    ...
    // Setup all variable with an initial value
    EndpointOUT = NULL;
    EndpointIN = NULL;
    u8EndpointIN = 0;
    u8EndpointOUT = 0;
    ...
    USB_ZERO_STRUCT(stcGlobalStorageInfo);
    pstcStorageInfo = NULL;
    USB_ZERO_STRUCT(setup);
    ...
    for(u8InterfaceNumber=0;u8InterfaceNumber < u8NumberOfInterfaces;u8InterfaceNumber++)
    {
        ...
        if (pu8Buffer == 0)
        {
            UsbHostMassStorage_DeinitHandler(pstcUsb);
            return FALSE;
        }
   ...
   {  
      ...
      if ((u32Timeout < 5) || (UsbHost_GetDeviceStatus(pstcUsbHandle) == USBHOST_DEVICE_IDLE))  // if there is no Handshake, something went wrong!
      {
         ...
         *NAKDetected = 1;   // There must be a circular NAK Handshake
         u32Timeout = 1000000;
         while((EndpointIN->bAbortTransfer) && (EndpointOUT->bAbortTransfer) && (u32Timeout > 5) && (UsbHost_GetDeviceStatus(pstcUsbHandle) != USBHOST_DEVICE_IDLE)) 
         {
             Usb_WaitHook();
             u32Timeout--;
         }
...
{  // Check if CSW is valid
    ...
    {
        ErrorHandler((uint16_t)0, &clrEponly);
        return USBH_STATUS_ERROR;
...
{
    UsbHost_ClearEndpointPoll(pstcUsbHandle, EndpointIN);
    /*
    ZERO_STRUCT(setup);
...
{
    UsbHost_ClearEndpointPoll(pstcUsbHandle, EndpointOUT);
                              /*
    ZERO_STRUCT(setup);
    setup.bmRequestType=0x02;  
    setup.bRequest=0x01;
