...
boolean_t UsbHostHidMouse_InitHandler(stc_usbn_t* pstcUsb, uint8_t* pu8Configuration, uint32_t u32Length)
{
    ...
    EndpointIN = NULL;
    USB_ZERO_STRUCT(stcInternalData);
    u8EndpointIN = 0;
    u16MaxPackageSizeIN = 0;
    USB_ZERO_STRUCT(stcUsbHostMouseData);
    bMouseMoved = FALSE;
    ...
    for(u8InterfaceNumber=0;u8InterfaceNumber < u8NumberOfInterfaces;u8InterfaceNumber++)
    {
        ...
        if (pu8Buffer == 0)
        {
            ...
            UsbHostHidMouse_DeinitHandler(pstcUsb);
            return FALSE;
        }
...
static void GetReportDescriptor(stc_usbn_t* pstcUsb, uint8_t u8Index, uint16_t u16Length, usbhost_setupcomplete_t pfnCallback)
{
    stc_usb_request_t stcSetup = {0x81,0x06,0x2200,0x00,0};
    stcSetup.wValue = 0x2200 + u8Index;
		stcSetup.wLength = u16Length;
